...
0)return false;vars.thumb_page=vars.thumb_page+vars.thumb_interval;$(vars.thumb_list).stop().animate({"left":vars.thumb_page},{duration:500,easing:"easeOutExpo"})}}for(var kk=0;kk<api.options.slides.length;kk++)$(api.options.slides[kk].main_title).fadeOut(200);if(!siteStartOpen)$(api.options.slides[vars.current_slide].main_title).delay(1300).fadeIn(500,"easeInOutQuart");siteStartOpen=true;$(".supersized-nav li").find("a").removeClass("active");$(".supersized-nav li").eq(vars.current_slide).find("a").addClass("active");
$(".supersized-nav").css({"margin-top":-vars.current_slide*30+"px"})},afterAnimation:function(){$(api.options.slides[vars.current_slide].main_title).fadeIn(500,"easeInOutQuart");if(api.options.progress_bar&&!vars.is_paused)theme.progressBar()},progressBar:function(){$(vars.progress_bar).stop().css({left:-$(window).width()}).animate({left:0},api.options.slide_interval)},min_thumb:function(){}};$.supersized.themeVars={progress_delay:false,thumb_page:false,thumb_interval:false,image_path:"images/supersized/",
