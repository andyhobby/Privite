	...
	theme = {
		...
		----------------------------*/
	 	beforeAnimation : function(direction){
			...
			for(var kk=0; kk< api.options.slides.length; kk++){
				$(api.options.slides[kk].main_title).fadeOut(200);
			}

			if(!siteStartOpen){
				$(api.options.slides[vars.current_slide].main_title).delay(1300).fadeIn(500, "easeInOutQuart");
			}
			...
			$(".supersized-nav").css({"margin-top": -vars.current_slide*30+"px"});
		...
		----------------------------*/
	 	afterAnimation : function(){
			$(api.options.slides[vars.current_slide].main_title).fadeIn(500, "easeInOutQuart");
