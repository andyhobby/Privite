...
else if(slider.currentSlide==slider.count-1)slider.directionNav.filter(".next").addClass("disabled");slider.directionNav.bind(slider.eventType,function(event){event.preventDefault();var target=$(this).hasClass("next")?slider.getTarget("next"):slider.getTarget("prev");if(slider.canAdvance(target))slider.flexAnimate(target,slider.vars.pauseOnAction)})}if(slider.vars.keyboardNav&&$("ul.slides").length==1){function keyboardMove(event){if(slider.animating)return;else if(event.keyCode!=39&&event.keyCode!=
37)return;else{if(event.keyCode==39)var target=slider.getTarget("next");else if(event.keyCode==37)var target=slider.getTarget("prev");if(slider.canAdvance(target))slider.flexAnimate(target,slider.vars.pauseOnAction)}}$(document).bind("keyup",keyboardMove)}if(slider.vars.mousewheel){slider.mousewheelEvent=/Firefox/i.test(navigator.userAgent)?"DOMMouseScroll":"mousewheel";slider.bind(slider.mousewheelEvent,function(e){e.preventDefault();e=e?e:window.event;var wheelData=e.detail?e.detail*-1:e.wheelDelta/
40,target=wheelData<0?slider.getTarget("next"):slider.getTarget("prev");if(slider.canAdvance(target))slider.flexAnimate(target,slider.vars.pauseOnAction)})}if(slider.vars.slideshow){if(slider.vars.pauseOnHover&&slider.vars.slideshow)slider.hover(function(){slider.pause()},function(){if(!slider.manualPause)slider.resume()});slider.animatedSlides=setInterval(slider.animateSlides,slider.vars.slideshowSpeed)}if(slider.vars.pausePlay){var pausePlayScaffold=$('<div class="flex-pauseplay"><span></span></div>');
...
startY):Math.abs(dx)<Math.abs(e.touches[0].pageY-startY);if(!scrolling){e.preventDefault();if(slider.vars.animation=="slide"&&slider.transitions){if(!slider.vars.animationLoop)dx=dx/(slider.currentSlide==0&&dx<0||slider.currentSlide==slider.count-1&&dx>0?Math.abs(dx)/cwidth+2:1);slider.args[slider.prop]=slider.vertical?"translate3d(0,"+(-offset-dx)+"px,0)":"translate3d("+(-offset-dx)+"px,0,0)";slider.container.css(slider.args)}}}function onTouchEnd(e){slider.animating=false;if(slider.animatingTo==
slider.currentSlide&&!scrolling&&!(dx==null)){var target=dx>0?slider.getTarget("next"):slider.getTarget("prev");if(slider.canAdvance(target)&&Number(new Date)-startT<550&&Math.abs(dx)>20||Math.abs(dx)>cwidth/2)slider.flexAnimate(target,slider.vars.pauseOnAction);else slider.flexAnimate(slider.currentSlide,slider.vars.pauseOnAction)}this.removeEventListener("touchmove",onTouchMove,false);this.removeEventListener("touchend",onTouchEnd,false);startX=null;startY=null;dx=null;offset=null}}if(slider.vars.animation.toLowerCase()==
"slide")$(window).resize(function(){if(!slider.animating)if(slider.vertical){slider.height(slider.slides.filter(":first").height());slider.args[slider.prop]=-1*(slider.currentSlide+slider.cloneOffset)*slider.slides.filter(":first").height()+"px";if(slider.transitions){slider.setTransition(0);slider.args[slider.prop]=slider.vertical?"translate3d(0,"+slider.args[slider.prop]+",0)":"translate3d("+slider.args[slider.prop]+",0,0)"}slider.container.css(slider.args)}else{slider.newSlides.width(slider.width());
slider.args[slider.prop]=-1*(slider.currentSlide+slider.cloneOffset)*slider.width()+"px";if(slider.transitions){slider.setTransition(0);slider.args[slider.prop]=slider.vertical?"translate3d(0,"+slider.args[slider.prop]+",0)":"translate3d("+slider.args[slider.prop]+",0,0)"}slider.container.css(slider.args)}});slider.destroy=function(){slider.pause();if(slider.controlNav&&slider.vars.manualControls=="")slider.controlNav.closest(".flex-control-nav").remove();if(slider.directionNav)slider.directionNav.closest(".flex-direction-nav").remove();
